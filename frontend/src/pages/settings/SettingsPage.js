...
function SettingsPage() {
  ...
  const fetchPrivacySettings = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.message || "Không thể lấy cài đặt quyền riêng tư."
        );
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/user/profile/${user.username}/privacy`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            customListId:
              settings.profileViewer === "custom"
                ? settings.customListId
                : null,
      ...
      toast.success(
        data.message || "Cài đặt quyền riêng tư đã được lưu thành công!"
      );
            ...
            <Col xs={12} lg={6} className="px-md-0">
              <div className="p-3">
                ...
                <Form>
                  ...
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-bold text-dark">
                      Ai có thể bình luận bài đăng của bạn?
                    </Form.Label>
                    <Form.Select
