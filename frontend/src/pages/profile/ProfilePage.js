...
function ProfilePage() {
  ...
  const renderPostsContent = () => (
      <div className="mt-0 border-top">
        ...
        {sampleTweets.length === 0 && <p className="text-muted text-center p-4">Không có bài đăng nào.</p>}
      </div>
  );

  const renderRepliesContent = () => (
      <div className="mt-0 border-top">
        {sampleReplies.map((tweet) => <TweetCard key={tweet.id} tweet={tweet} />)}
        {sampleReplies.length === 0 && <p className="text-muted text-center p-4">Không có phản hồi nào.</p>}
      </div>
  );

  const renderMediaContent = () => (
      <div className="mt-0 border-top">
        {sampleMedia.map((tweet) => <TweetCard key={tweet.id} tweet={tweet} />)}
        {sampleMedia.length === 0 && <p className="text-muted text-center p-4">Không có phương tiện nào.</p>}
      </div>
  );

  const renderLikesContent = () => (
      <div className="mt-0 border-top">
        {sampleLikes.map((tweet) => <TweetCard key={tweet.id} tweet={tweet} />)}
        {sampleLikes.length === 0 && <p className="text-muted text-center p-4">Không có lượt thích nào.</p>}
      </div>
  );

  const renderHighlightsContent = () => (
      <div className="mt-0 border-top">
        {sampleHighlights.map((tweet) => <TweetCard key={tweet.id} tweet={tweet} />)}
        ...
        {sampleArticles.map((article) => (
            <div key={article.id} className="border-bottom p-3 hover-bg-light">
              ...
              <div className="d-flex justify-content-between text-muted small">
                <span>{new Date(article.timestamp).toLocaleDateString("vi-VN")}</span>
      ...
      <div className="d-flex min-vh-100" style={{ backgroundColor: "#fff" }}>
        ...
        <div className="d-flex flex-column flex-grow-1">
          <div className="sticky-top bg-white border-bottom py-2" style={{ zIndex: 1020 }}>
            <Container fluid>
              <Row>
                <Col xs={12} md={8} lg={12} xl={12} className="mx-auto d-flex align-items-center ps-md-0">
                  <Link to="/" className="btn btn-link text-dark me-3">
                    <FaArrowLeft size={20} />
                  </Link>
                  <div>
                    <h5 className="mb-0 fw-bold">{userProfile.name}</h5>
                    <p className="text-muted small mb-0">{userProfile.postCount} bài đăng</p>
                  </div>
                </Col>
              </Row>
            </Container>
