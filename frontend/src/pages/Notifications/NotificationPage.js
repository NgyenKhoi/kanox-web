...
function NotificationPage() {
  ...
  const notifications = [
    ...
    },
    {
      id: 3,
      type: "community",
      content: "Ghi chép cho cộng đồng. Người đọc đã thêm Ghi chép cho cộng đồng vào một bài đăng mà bạn đã thích, trả lời hoặc đăng lại",
  ...
  const trendingTopics = [
    { id: 1, category: "Chủ đề nổi trội ở Việt Nam", title: "Success", posts: 210 },
    { id: 2, category: "Chủ đề nổi trội ở Việt Nam", title: "#riyadh", posts: "1,12 Tr" },
    { id: 3, category: "Chủ đề nổi trội ở Việt Nam", title: "#khảo_oam", posts: 210 },
    { id: 4, category: "Chủ đề nổi trội ở Việt Nam", title: "Inspiring", posts: "46,3 N" },
  ];
                  ...
                  <div key={notification.id} className="p-3 border-bottom">
                    {notification.type === "mention" && (
                        <div className="d-flex align-items-start">
                          <Image src={notification.avatar} roundedCircle className="me-2" style={{ width: "40px", height: "40px" }} />
                          <div>
                            ...
                            {notification.tags && (
                                <p className="text-primary small mb-1">{notification.tags.map((tag, index) => <span key={index} className="me-1">{tag}</span>)}</p>
                            )}
                            {notification.image && notification.image.startsWith("http") && (
                                <Image src={notification.image} fluid className="mt-2" style={{ maxWidth: "100%", height: "auto" }} />
                            )}
          ...
          <div className="sticky-top bg-white border-bottom py-2" style={{ zIndex: 1020 }}>
            <Container fluid>
              ...
              <Nav variant="underline" className="mt-2 nav-justified notification-tabs">
                <Nav.Item><Nav.Link onClick={() => setActiveTab("all")} className={`text-dark fw-bold ${activeTab === "all" ? "active" : ""}`}>Tất cả</Nav.Link></Nav.Item>
                <Nav.Item><Nav.Link onClick={() => setActiveTab("verified")} className={`text-dark fw-bold ${activeTab === "verified" ? "active" : ""}`}>Đã xác nhận</Nav.Link></Nav.Item>
                <Nav.Item><Nav.Link onClick={() => setActiveTab("mentions")} className={`text-dark fw-bold ${activeTab === "mentions" ? "active" : ""}`}>Đề cập</Nav.Link></Nav.Item>
              </Nav>
            </Container>
          </div>
