...
import { FaCheckCircle, FaCircle, FaEllipsisH, FaCog } from "react-icons/fa";
...
function NotificationPage() {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      ...
      try {
        ...
        // Đảm bảo dữ liệu từ API là mảng
  ...
  // SỬA: Xử lý thông báo từ WebSocket và cập nhật state
  useWebSocket(
      (notification) => {
        setNotifications((prev) => {
          ...
          if (!exists) {
            return [{ ...notification, isRead: notification.status === "read" }, ...prev];
          }
          return prev.map((n) =>
  ...
  const renderNotificationContent = () => {
    ...
    return notifications.map((notification) => (
        <div
            key={notification.id}
            className={`p-3 border-bottom ${notification.status === "unread" ? "fw-bold text-dark" : "text-muted"}`}
            style={{ backgroundColor: notification.status === "unread" ? "#f8f9fa" : "transparent" }}
        >
