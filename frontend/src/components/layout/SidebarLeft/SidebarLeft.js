          ...
          <Dropdown drop="up" className="w-100">
            ...
            >
              ...
              <div className="d-none d-lg-block flex-grow-1">
                <div className="fw-bold text-dark">{user?.name || "Người dùng"}</div>
                <div className="text-muted small">@{user?.username || "username"}</div>
              </div>
              <FaEllipsisH className="ms-auto me-2 text-dark d-none d-lg-block" />
            </Dropdown.Toggle>
            <Dropdown.Menu className="sidebar-user-dropdown-menu">
              <Dropdown.Item
                  onClick={() => navigate(`/profile/${user?.username || "default"}`)}
              >
                Xem hồ sơ
              </Dropdown.Item>
              <Dropdown.Item
