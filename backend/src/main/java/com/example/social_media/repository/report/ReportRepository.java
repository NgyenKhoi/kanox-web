...
public interface ReportRepository extends JpaRepository<Report, Integer> {
    ...
    @Procedure(procedureName = "sp_UpdateUserLockStatus", outputParameterName = "return_code")
    ...
    @EntityGraph(attributePaths = {"reporter", "targetType", "reason", "processingStatus"})
    @Query("SELECT r FROM Report r JOIN Post p ON r.targetId = p.id WHERE p.group.id = :groupId AND r.status = :status")
    Page<Report> findByGroupIdAndTargetTypeId(@Param("groupId") Integer groupId, @Param("status") Boolean status, Pageable pageable);
}