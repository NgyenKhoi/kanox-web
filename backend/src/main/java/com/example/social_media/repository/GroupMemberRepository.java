...
public interface GroupMemberRepository extends JpaRepository<GroupMember, GroupMemberId> {
    ...
    @Query("SELECT gm FROM GroupMember gm JOIN gm.user u " +
            "WHERE gm.id.groupId = :groupId AND gm.status = true AND gm.inviteStatus = 'ACCEPTED' " +
    ...
    boolean existsById_GroupIdAndId_UserIdAndInviteStatus(Integer groupId, Integer userId, String inviteStatus);
    ...
    @Query("SELECT gm FROM GroupMember gm WHERE gm.id.groupId = :groupId AND gm.isAdmin = true")
    List<GroupMember> findById_GroupIdAndIsAdminTrue(@Param("groupId") Integer groupId);
}
